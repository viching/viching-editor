...
const materialOutPath = join(outputDir, 'packages', 'viching-editor');
...
task('serve:devapp', ['build:devapp'], sequenceTask(
  [':serve:devapp', 'viching-editor:watch', ':watch:devapp']
));
...
task('stage-deploy:devapp', ['build:devapp'], () => {
  ...
  copyFiles(materialOutPath, '**/prebuilt/*.+(css|map)', join(outDir, 'dist/packages/viching-editor'));
});
