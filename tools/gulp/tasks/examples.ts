...
const examplesPath = path.join(packagesDir, 'viching-editor-examples');
...
function buildImportsTemplate(metadata: ExampleMetadata): string {
  ...
  const relativeSrcPath = path
    .relative('./src/viching-editor-examples', metadata.sourcePath)
    .replace(/\\/g, '')
    .replace('.ts', '');
...
import {ExampleMaterialModule} from './viching-editor-module';
...
task('build-examples-module', () => {
  ...
  for (const sourcePath of matchedFiles) {
    ...
    const { primaryComponent, secondaryComponents } =
      parseExampleMetadata(sourcePath, <string>sourceContent);
